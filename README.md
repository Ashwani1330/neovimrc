...
Open **PowerShell as Administrator** and run the followi# ⚡ Neovim Configuration for Windows
...
Make sure you are using a **Nerd Font** in your terminal (e.g., Windows Terminal settings).ng commands:
...
| Key | Action |
| --- | --- |
| `<leader>pv` / `<leader>e` | Open File Explorer (Oil.nvim) |
| `<leader>ff` | Find Files (Telescope) |
| `<leader>fg` | Live Grep (Search text in files) |
| `<leader>fb` | Switch Buffers |
| `Ctrl+h/j/k/l` | Navigate between windows |
...
| Key | Action |
| --- | --- |
| `Ctrl+J` | Accept Ghost Text Suggestion |
| `<leader>aa` | Toggle Copilot Chat Sidebar |
| `<leader>aq` | Quick Chat (Input window) |
| `<leader>ae` | Explain Code (Visual Mode) |
| `<leader>af` | Fix Bug (Visual Mode) |
